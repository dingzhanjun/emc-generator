...
class LandstarbrokerGenerator
{
	...
	public function execute()
	{
		...
		if (!$config) {
			$notify_error = new NotifyError("Landstarbroker - Config not found\n");
		...
		if (!$jobboard) {
			$notify_error = new NotifyError("Landstarbroker - Jobboard not found\n");
			$notify_error->execute();
			return;
		}
		...
		try {
			...
			$notify_error = new NotifyError("Landstarbroker - Jobboard have been changed. Please contact to VTNS\n");
			$notify_error->execute();
		}
	}
